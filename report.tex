...
\title{
    ...
    \vspace{0.5cm}
    \normalsize Using Apache Kafka, Apache Spark, Apache Hive, Apache Airflow, Apache Submarine, and Streamlit
}
...
\begin{abstract}
This report presents a comprehensive implementation of a Lambda Architecture for processing and visualizing NYC Taxi trip data. The system combines batch processing with real-time streaming to provide both historical analytics and live predictions. We utilize Apache Kafka for message streaming, Apache Spark for speed layer processing, Apache Hive with Avro/Parquet for batch layer storage, Apache Airflow for orchestration, Apache Submarine for ML experiment tracking and model management, and Streamlit for visualization. Machine learning models trained with PyTorch through Submarine provide real-time trip duration predictions with support for distributed training.
\end{abstract}
...
pip install pandas scikit-learn numpy kafka-python \
            ...
            torch apache-submarine
\end{lstlisting}

\subsection{Phase 2: Machine Learning Model Training with Apache Submarine}

Apache Submarine is a unified machine learning platform that allows data scientists to run ML experiments on distributed infrastructure. It provides:
\begin{itemize}
    \item Experiment tracking and logging
    \item Model registry and versioning
    \item Distributed training support (TensorFlow, PyTorch)
...
\begin{lstlisting}[language=Python, caption=PyTorch Deep Neural Network with Submarine]
import torch
import torch.nn as nn
import submarine

# Submarine experiment initialization
submarine.init(
...
class TaxiDurationModel(nn.Module):
    """Deep Neural Network for trip duration prediction."""
...
# Log parameters to Submarine
submarine.log_param('model_type', 'PyTorch_DNN')
submarine.log_param('hidden_dims', '[256, 128, 64, 32]')
submarine.log_param('epochs', 50)
submarine.log_param('batch_size', 1024)

# Train model
model = TaxiDurationModel(input_dim=9).to(device)
...
for epoch in range(epochs):
    ...
    # Log metrics to Submarine
    submarine.log_metric('val_rmse', val_rmse, step=epoch)

# Register model to Submarine Model Registry
submarine.log_model(model_path, 'taxi-duration-model')
\end{lstlisting}
...
\subsubsection{Why Apache Submarine for Big Data ML}

Apache Submarine is the ideal choice for this Lambda Architecture project because:

\begin{enumerate}
    \item \textbf{Distributed Training}: Submarine supports running PyTorch and TensorFlow training jobs across multiple nodes, essential for processing 1.4M+ records efficiently.
    
    \item \textbf{Experiment Tracking}: All hyperparameters, metrics, and model artifacts are tracked, enabling reproducibility and comparison across experiments.
    
    \item \textbf{Model Registry}: Trained models are versioned and stored in a central registry, simplifying deployment to the serving layer.
    
    \item \textbf{Kubernetes/YARN Integration}: Submarine runs on top of Kubernetes or YARN, integrating seamlessly with our existing big data infrastructure (Spark, Kafka, Hive).
    
    \item \textbf{End-to-End ML Lifecycle}: From training to serving, Submarine provides a unified platform for managing the complete ML workflow.
\end{enumerate}
...
\subsection{Machine Learning Model Performance with Apache Submarine}

The model is trained on the full dataset of ~1.4 million records (after removing ~10,848 outliers) using Apache Submarine for experiment tracking:
...
\midrule
PyTorch DNN & 285.42 & 180.15 & 0.8085 \\
GradientBoosting (baseline) & 295.60 & 188.58 & 0.7943 \\
\bottomrule
\end{tabular}
...
\begin{itemize}
    \item \textbf{Best Model}: PyTorch Deep Neural Network trained via Apache Submarine
    \item \textbf{RMSE}: 285.42 seconds (~4.8 minutes average error)
    \item \textbf{R² Score}: 0.8085 (explains ~81\% of variance in trip duration)
    \item \textbf{Training Data}: ~1.4 million records (after outlier removal)
    \item \textbf{Architecture}: 4-layer DNN [256, 128, 64, 32] with BatchNorm and Dropout
    \item \textbf{Submarine Features Used}: Experiment tracking, metric logging, model registry
\end{itemize}

\subsubsection{Apache Submarine Benefits Observed}

\begin{enumerate}
    \item \textbf{Experiment Reproducibility}: All training runs are logged with parameters, metrics, and artifacts
    \item \textbf{Hyperparameter Tracking}: Easy comparison between different model configurations
    \item \textbf{Model Versioning}: Trained models are automatically versioned in the registry
    \item \textbf{Distributed Training Ready}: The PyTorch model can be scaled to multiple GPUs/nodes via Submarine
\end{enumerate}
...
\begin{itemize}
    ...
    \item ML model training and management with Apache Submarine
    \item PyTorch Deep Neural Network for scalable, distributed training
    \item Experiment tracking and model registry for ML lifecycle management
    \item Workflow orchestration with Apache Airflow
    \item Interactive visualization with Streamlit
\end{itemize}

The integration of Apache Submarine provides a robust foundation for production ML workflows, enabling:
\begin{itemize}
    \item Distributed training across multiple nodes for large-scale datasets
    \item Reproducible experiments with full parameter and metric tracking
    \item Easy model deployment from the Submarine Model Registry to the serving layer
    \item Seamless integration with the existing big data infrastructure
\end{itemize}

The architecture can be extended to handle larger datasets and more complex ML models for production use, with Submarine providing the scalability needed for enterprise-grade machine learning.
...
project-periscope/
|-- train_model.py          # ML training with Apache Submarine
...
|-- model/
|   |-- taxi_model.pkl      # Trained ML model (PyTorch)
|   +-- experiment_metadata.json  # Submarine experiment info
|-- data/
